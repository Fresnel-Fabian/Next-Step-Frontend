...
export default function TeachersScreen() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    await Promise.all([fetchTeachers(), fetchInvitations()]);
    setLoading(false);
  };

  const fetchTeachers = async () => {
    try {
      const res = await api.get('/api/v1/users?role=TEACHER');
      setTeachers(res.data);
    } catch {
      Toast.show({ type: 'error', text1: 'Failed to load teachers', position: 'top' });
    }
  };

  const fetchInvitations = async () => {
    try {
      const res = await api.get('/api/v1/invitations?role=TEACHER');
  ...
  const handleInvite = async () => {
    ...
    try {
      setInviting(true);
      const res = await api.post('/api/v1/invitations/invite', { email: email.trim(), role: 'TEACHER' });
      setEmail('');
      setPendingInvites(prev => [res.data, ...prev]);
      Toast.show({ type: 'success', text1: 'Teacher invited!', text2: res.data.email, position: 'top' });
    } catch (error: any) {
  ...
  const handleCSVUpload = async () => {
    try {
      ...
      const res = await api.post('/api/v1/invitations/invite/bulk?role=TEACHER', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      await fetchInvitations();
      Toast.show({
        type: 'success',
        text1: `${res.data.invited.length} teacher(s) invited`,
  ...
  const openRemoveModal = (teacher: Teacher) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await api.delete(`/api/v1/users/${selectedTeacher.id}`);
      setTeachers(prev => prev.filter(t => t.id !== selectedTeacher.id));
      Toast.show({ type: 'success', text1: 'Teacher deleted permanently', position: 'top' });
    } catch {
      Toast.show({ type: 'error', text1: 'Failed to delete teacher', position: 'top' });
    } finally {
  ...
  const handleDeactivate = async () => {
    ...
    try {
      setActionLoading(true);
      await api.patch(`/api/v1/users/${selectedTeacher.id}/deactivate`);
      setTeachers(prev =>
        prev.map(t => t.id === selectedTeacher.id ? { ...t, is_active: false } : t)
      );
      Toast.show({ type: 'success', text1: 'Teacher deactivated', position: 'top' });
    } catch {
      Toast.show({ type: 'error', text1: 'Failed to deactivate teacher', position: 'top' });
    } finally {
      setActionLoading(false);
      setModalVisible(false);
      setSelectedTeacher(null);
    ...
    <View style={styles.flex}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>Invite and manage teacher accounts</Text>
        </View>
        ...
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <View>
              <Text style={styles.cardTitle}>Invite Teacher</Text>
...
const styles = StyleSheet.create({
  ...
  inviteBtn: {
    backgroundColor: '#7C3AED', paddingHorizontal: 18, height: 44,
  ...
  modalName: { fontWeight: '600', color: '#111827' },
